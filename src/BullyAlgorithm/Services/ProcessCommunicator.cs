...
namespace BullyAlgorithm.Services
{
    public class ProcessCommunicator
    {
        ...
        public CommunicatorMessage Send(int toProcessId, CommunicatorMessage message,bool recieve=true,int timeOut=0)
        {
        ...
        public bool Recieve(MessageTypes type,int timeOut=0)
        {
            var handler = _listener.Accept();
        ...
        public void Run()
        {
            ...
            {
                _isCoordinator = true;
                _messageWritter.Write($"[{DateTime.UtcNow} - [{_processId}] ] Process ({_processId}) won the election and became the coordinator :)");
            }
            while (_isActive)
            {
                RecieveMessages(1000);
        ...
        private void HandleMessage(CommunicatorMessage message)
        {
            if(message.Type==MessageTypes.Join)
            {
                ...
                _messageWritter.Write($"[{DateTime.UtcNow} - [{_processId}] ] Processe ({message.From}) Joined the cluster");
            }else if(message.Type==MessageTypes.Coordinator)
            {
                _messageWritter.Write($"[{DateTime.UtcNow} - [{_processId}] ] Process ({message.From}) is the coordinator ");
                _isCoordinator = false;

            }else if (message.Type == MessageTypes.Election)
            {

                _messageWritter.Write($"[{DateTime.UtcNow} - [{_processId}] ] Election Message received from Process ({message.From})");
            ...
            else if(message.Type==MessageTypes.HeartBeat)
            {
                ...
                _messageWritter.Write($"[{DateTime.UtcNow} - [{_processId}] ] HeartBeat received from Process ({message.From})");
            }else if(message.Type==MessageTypes.Shutdown)
            {
                _messageWritter.Write($"[{DateTime.UtcNow} - [{_processId}] ] Process ({message.From}) is Shuting down");
            }
        }
        ...
        private void JoinToCluster()
        {
            ...
            try
            {
                ...
                _messageWritter.Write($"[{DateTime.UtcNow} - [{_processId}] ] Processe ({_processId}) is Joining the cluster...");
        ...
        {
            _messageWritter.Write($"[{DateTime.UtcNow} - [{_processId}] ] Process ({_processId}) is starting an election");
            _isCoordinator = false;
            ...
            if(!recievedOk)
            {
                ...
                foreach(var process in _clustrProcesses)
                {
                    Send(process, coordinatorMessage, false);
                }
            }else
            {
                bool recievedCoordinatorMessage = Recieve(MessageTypes.Coordinator, 1000);
